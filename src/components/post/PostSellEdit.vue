...
<div class="px-4 py-5 flex-auto">
    <div class="flex-auto lg:px-10 py-10 pt-0">
        <form @submit.prevent="updateProduct()">
            ...
            <v-select dense rounded :items="choices.sell_type" item-text="name" item-value="id" v-model="product.sell_type" class="w-full " filled :label="_lang('ประเภทการขาย','Sales type','銷售類型')"></v-select>
            <v-text-field dense rounded v-if="product.sell_type == 0" type="date" v-model="product.buy_date" class="w-full " filled :label="_lang('วันที่สิ้นสุดประกาศขาย','Sale end date','銷售結束日期')"></v-text-field>
