...
<div class="px-4 py-5 flex-auto">
    <div class="flex-auto lg:px-10 py-10 pt-0">
        <form @submit.prevent="storeProduct()">
            ...
            <v-text-field rounded v-if="!product.price_type" type="text" v-model="product.price_end" class="w-full " filled :label="_lang('ราคาสุดท้าย','PriceEnd','价格结束')"></v-text-field>
            ...
            <v-text-field rounded v-if="product.sell_type == 0" type="date" v-model="product.buy_date" class="w-full pt-3" filled :label="_lang('วันที่สิ้นสุดประกาศซื้อ','Sale end date','銷售結束日期')"></v-text-field>
            ...
            <v-btn rounded type="submit" large color="success" class="w-full"><v-icon>mdi-floppy</v-icon> {{_lang('บันทึก','Save','保存')}}</v-btn>
        </form>
    </div>
</div>
</template>
