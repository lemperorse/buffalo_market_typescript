...
<div class="">
    ...
    <v-card class="rounded-lg p-2 mt-2 overflow-hidden ">
        <div class=" m-4 ">
            <v-row wrap class="md:justify-center md:items-center">
                ...
                <v-col cols="6" sm="6" md="4">
                    <div>
                        ...
                        <div class="flex wrap">
                            <v-btn @click="$router.push(`/user/productall?product=${product.id}&name=${product.farm.user.first_name}`)" rounded type="submit" class="md:w-1/2 bgbtn p-2" dark>
                                <v-icon>mdi-shopping-search</v-icon> {{_lang('ดูสินค้า','View','查看產品')}}
                            </v-btn>
                            <v-divider vertical class="mr-2 ml-2"></v-divider>
                            <v-btn @click="openMap" rounded type="submit" class=" md:w-1/2 bgbtn p-2" dark>
                                <v-icon>mdi-map-marker-radius-outline</v-icon> {{_lang('นำทาง','Navigate','導航')}}
                            </v-btn>
                        </div>
                    </div>
                </v-col>
                ...
                <v-tabs class="rounded" color="teal accent-3" next-icon="mdi-arrow-right-bold-circle-outline" prev-icon="mdi-arrow-left-bold-circle-outline" show-arrows>
                    ...
                    <v-tab-item>
                        ...
                        <p class="font-weight-thin text-md">
                            <v-icon>mdi-facebook</v-icon> Facebook : <span>{{product.farm.facebook}}</span>
                        </p>
                        <p class="font-weight-thin text-md"><i class="fab fa-line text-xl pl-1 pr-1"></i> Line : <span>{{product.farm.line}}</span></p>
                        <p class="font-weight-thin text-md">
                            <v-icon>mdi-cellphone</v-icon> {{_lang('เบอร์ติดต่อ','Phone number','電話號碼')}} : <span>{{product.farm.tel}}</span>
                        </p>
                        <p class="font-weight-thin text-md">
