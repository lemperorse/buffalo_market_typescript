            ...
            <v-expansion-panels flat multiple v-model="panel" class="m-2 bg-transparent">
                <v-expansion-panel>
                    <v-expansion-panel-header>
                        <h2 class="text-xl">
                            <i class="text-2xl em em-card_file_box" aria-role="presentation" aria-label=""></i>
                ...
                ">
                        ...
                        <p class="text-yellow-600 rounded-full p-3 text-xl font-bold">
                            <i class="em em-money_with_wings" aria-role="presentation" aria-label="MONEY WITH WINGS"></i>
                            {{ _lang("ราคา", "Price", "價錢") }}
                        </p>
                        <div class="flex p-3 text-xl">
                            <v-text-field v-model="price_low" name="name" id="id" :label="_lang('ต่ำสุด', 'Lowest', '最低的')"></v-text-field>
                            -
                            <v-text-field v-model="price_height" name="name" id="id" :label="_lang('สูงสุด', 'Maximum', '最大')"></v-text-field>
                            <v-btn rounded color="orange" outlined class="mt-2 text-yellow-600" @click="changepriceType" depressed>{{ _lang("ตกลง", "OK", "搜索") }}</v-btn>
                        </div>
                        <p class="text-yellow-600 rounded-full p-3 text-xl font-bold">
                            <i class="em em-clock1" aria-role="presentation" aria-label="CLOCK FACE ONE OCLOCK"></i>
        ...
        <div :class="`w-full ${
          advanceSearch ? 'md:w-3/4' : ''
            ...
            advanceSearch ? 'lg:grid-cols-6' : 'lg:grid-cols-7'
                ...
                <le-card v-for="(pu, i) in products.results" :key="i" classProp="w-full" :path="`/user/productdetail?product=${pu.id}&name=${pu.name}`" :image="ximg(pu.file1)" :name="pu.name" :price="
