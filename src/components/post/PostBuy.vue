    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <v-row dense>
            <v-col cols="12" sm="8" class="flex flex-wrap">
                ...
                <hr class="m-1">
                <v-btn type="submit " depressed @click="loadProduct" large fab class="w-full" color="success">
                    <v-icon dark>fas fa-search</v-icon>
                </v-btn>
                <!-- {{_lang('ค้นหา','Search','搜索')}} -->
            </v-col>

            <v-col cols="12" sm="4" class="relative ">
                <div class="text-right">
                    <v-btn type="submit " @click="$router.push(`/user/addpostbuy`)" large rounded class="w-full md:w-auto float-md-right" color="success">
                        <v-icon dark>far fa-plus-square</v-icon>{{_lang('เพิ่มประกาศซื้อ','Add product','添加產品')}}
                    </v-btn>
                </div>
            </v-col>
        </v-row>
                ...
                <v-hover v-slot:default="{ hover }">
                    <v-card class="rounded-lg " outlined>
                        ...
                        <div class="p-4">
                            ...
                            <div class=" flex ">
                                <v-btn type="submit " @click="$router.push(`/user/postbuyedit/?product=${product.id}`)" large rounded class="w-full" color="success">
