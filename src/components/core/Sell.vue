...
<div class="pt-6">
    <div class="flex flex-col md:flex-row">
        <div class="w-full md:w-1/5 p-2 animate__animated  animate__bounceInLeft" v-if="advanceSearch">
            <v-expansion-panels flat multiple v-model="panel" class=" m-2 bg-transparent">
                <v-expansion-panel>
                    <v-expansion-panel-header>
                        <h2 class="text-xl"> <i class="text-2xl em em-card_file_box" aria-role="presentation" aria-label=""></i>
                    ...
                    <v-expansion-panel-content class="pt-4 rounded-lg ">
                        ...
                        <p class="p-3 text-xl font-bold">
                            {{_lang('ประกาศขาย','Sell announcement','出售公告')}}
                        </p>

                        <p class="text-yellow-600 rounded-full p-3 text-xl font-bold">
                            <i class="em em-flag-th" aria-role="presentation" aria-label="Thailand Flag"></i> {{_lang('จังหวัด','Province','省')}}
                        </p>

                        <v-autocomplete class="p-3 text-xl" item-text="name" item-value="id" @change="loadProducts()" :label="_lang('จังหวัด','Province','省')" :items="provinces" v-model="province"></v-autocomplete>
                        <p class="text-yellow-600 rounded-full p-3 text-xl font-bold">
                            <i class="em em-money_with_wings" aria-role="presentation" aria-label="MONEY WITH WINGS"></i> {{_lang('ราคา','Price','價錢')}}
                        </p>
                        <div class="flex p-3 text-xl">
                            ...
                            <v-btn rounded color="orange" outlined class="mt-2 text-yellow-600" @click="changepriceType" depressed>{{_lang('ตกลง','OK','搜索')}}</v-btn>
                        </div>
                        <p class="text-yellow-600 rounded-full p-3 text-xl font-bold">
                            <i class="em em-clock1" aria-role="presentation" aria-label="CLOCK FACE ONE OCLOCK"></i> {{_lang('ระยะเวลาการขาย','Time period','時間段')}}
                        </p>
                        ...
                        <div v-for="category,index in categories" :key="index" class="pb-6">
                            <h2 class="text-yellow-600 rounded-full p-3 text-base">{{category.name}}</h2>
                        ...
                        <v-btn @click="resetPage()" rounded block color="orange" outlined class="text-yellow-600">{{_lang('ลบทั้งหมด','Clean up','清理')}}</v-btn>
                    </v-expansion-panel-content>
                </v-expansion-panel>

            </v-expansion-panels>
        </div>
        <div :class="`w-full ${(advanceSearch)?'md:w-3/4':''} md:pl-6 animate__animated  animate__bounceInRight` ">
            <div class="w-full p-6">
                <v-toolbar color="transparent" flat>
                    <div class="flex items-center h-auto">
                        <i class="mr-4 text-xl md:text-5xl em em-shopping_trolley" aria-role="presentation" aria-label=""></i>
                        <div class="pt-6">
                            <h2 class="text-xl md:text-4xl font-bold"> {{_lang('ประกาศ ขาย','Sell announcement','出售公告')}}</h2>
                            <h2 class="text-xl md:text-2xl font-bold text-red-500"> {{_lang('หากคุณต้องการซื้อสินค้า','If you want to buy','如果你想买')}}</h2>
                            <p>{{_lang('การประกาศขาย สินค้าต่างๆ ในระบบ','Announcing the sale of various products in the system','宣布系统内各种产品的销售')}} </p>
                        </div>
                    </div>
                    <v-spacer></v-spacer>
                    <v-btn v-if="!$vuetify.breakpoint.mobile" outlined rounded @click="advanceSearch = !advanceSearch" color="orange">

                        <span v-if="!advanceSearch">
                            <v-icon>mdi-card-search</v-icon> ค้นหาแบบละเอียด
                        </span>
