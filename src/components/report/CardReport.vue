<template>
<div> 
    <div class="grid gap-6 mb-6 mt-6 md:grid-cols-2 xl:grid-cols-4">
        <!-- รายได้ทั้งหมด -->
        <div class="flex items-center p-4 bg-white border-l-8 border-red-500 rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-red-500 rounded-full">
                <i class="fas fa-clipboard-list text-4xl" aria-role="presentation" aria-label="CHEERING MEGAPHONE"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    รายได้ทั้งหมด 
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    ฿ 30,000
                </p>
            </div>
        </div>
        <!-- ประกาศการขาย -->
        <div class="flex items-center p-4 bg-white border-l-8 border-blue-500 rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-blue-500  rounded-full ">
                <i class="fas fa-clipboard-list text-4xl" aria-role="presentation" aria-label="CHEERING MEGAPHONE"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    ประกาศทั้งหมด
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    30 ประกาศ
                </p>
            </div>
        </div>
        <!-- ประกาศการขาย -->
        <div class="flex items-center p-4 bg-white border-l-8 border-purple-500 rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-purple-500 rounded-full ">
                <i class="fas fa-clipboard text-4xl" aria-role="presentation" aria-label="CHEERING MEGAPHONE"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    ประกาศการขาย
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    30 ประกาศ
                </p>
            </div>
        </div>
        <!-- ปิดการขาย -->
        <div class="flex items-center p-4 bg-white border-l-8 border-green-500 rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-green-500 rounded-full ">
                <i class="fas fa-clipboard-check text-4xl" aria-role="presentation" aria-label="MONEY WITH WINGS"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    ปิดการขาย
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    23 ประกาศ
                </p>
            </div>
        </div>
        <!-- ประกาศการรับซื้อ -->
        <div class="flex items-center p-4 bg-white border-l-8 border-gray-500 rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-gray-500 rounded-full  ">
                <i class="far fa-clipboard text-4xl" aria-role="presentation" aria-label="NEWSPAPER"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    ประกาศการรับซื้อ
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    10 ประกาศ
                </p>
            </div>
        </div>
        <!-- ปิดการรับซื้อ -->
        <div class="flex items-center p-4 bg-white border-l-8 border-black rounded-lg shadow-xl">
            <div class="p-3 mr-4 text-black rounded-full">
                <i class="fas fa-clipboard-check text-4xl" aria-role="presentation" aria-label="PALM TREE"></i>
            </div>
            <div>
                <p class="mb-2 text-sm font-medium text-gray-600 dark:text-gray-400">
                    ปิดการรับซื้อ
                </p>
                <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                    7 ประกาศ
                </p>
            </div>
        </div>
    </div>
</div>
</template>

<script>
export default {

}
</script>

<style>

</style>
