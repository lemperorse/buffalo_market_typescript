    ...
    <div class="flex-auto lg:px-10 py-10 pt-0" v-if="response">
      <form @submit.prevent="storeProduct()">
        ...
        <v-text-field
          ...
          class="w-full"
          filled
          :label="_lang('ชื่อสินค้า', 'Product name', '產品名稱')"
        ...
        <v-select
          ...
          dense
          rounded
          :items="choices.price_type"
          item-text="name"
          item-value="id"
          v-model="product.price_type"
          class="w-full"
          filled
          :label="_lang('ประเภทราคา', 'Price Type', '价格类型')"
        ...
        <v-text-field
          ...
          v-if="!product.price_type"
          type="text"
          v-model="product.price_start"
          class="w-full"
          filled
          :label="_lang('ราคาเริ่มต้น', 'PriceStart', '价格开始')"
          prepend-inner-icon="mdi-currency-btc"
        ></v-text-field>
        <v-text-field
          required
          ...
          v-if="!product.price_type"
          type="text"
          v-model="product.price_end"
          class="w-full"
          filled
          :label="_lang('ราคาสุดท้าย', 'PriceEnd', '价格结束')"
          prepend-inner-icon="mdi-currency-btc"
        ></v-text-field>
        <v-text-field
          required
          ...
          filled
          :label="_lang('วันส่งมอบ', 'Delivery date', '郵寄日期')"
        ...
        <v-textarea
          ...
          class="w-full"
          filled
          :label="_lang('รายละเอียด', 'Detail', '详情')"
          prepend-inner-icon="mdi-clipboard-text-outline"
        ></v-textarea>
        <p class="text-red-600">
          * จำกัดไฟล์อัพโหลดได้สูงสุดที่ 5 ไฟล์ โดย ไฟล์ที่ 1,2,3
          ต้องเป็นไฟล์รูปเท่านั้น และ ไฟล์ที่ 4,5 เป็นไฟล์วีดีโอเท่านั้น
          (ไม่จำเป็นต้องอัพโหลดครบทั้ง 5 ไฟล์)
        </p>
        <VueFileAgent
          required
          v-model="file"
        ...
        <v-select
          ...
          dense
          rounded
          :items="choices.sell_type"
          item-text="name"
          item-value="id"
          v-model="product.sell_type"
          class="w-full pt-3"
          filled
          :label="_lang('ประเภทการซื้อ', 'Sales type', '銷售類型')"
          prepend-inner-icon="mdi-calendar-alert"
        ></v-select>
        <v-text-field
          required
          rounded
          v-if="product.sell_type == 0"
          type="date"
          v-model="product.buy_date"
          class="w-full pt-3"
          filled
          :label="
        ...
        <v-select
          ...
          class="w-full"
          filled
          :label="_lang('สถานะประกาศ ', 'Announcement Status', '公告狀態')"
          prepend-inner-icon="mdi-tag-multiple-outline"
        ></v-select>
        <div class="">
          <div v-for="(category, index) in categories" :key="index">
            <div class="">
              <v-icon>mdi-tag-outline</v-icon> {{ category.name }}
            </div>
            <v-checkbox
              class="-mb-6"
