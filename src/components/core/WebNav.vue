...
<div class="fixed w-full h-32 bg-beta" style="z-index:100;">
    <div class=" p-2 md:pl-20 md:pr-20 mt-6 md:pt-0">
        <div class="flex row-wrap overflow-y-hidden	overflow-x-auto p-1 " v-if="response">
            ...
            <v-btn @click="$router.push('/ProductBuy')" small text dark>
                <v-icon>mdi-shopping-outline</v-icon>{{_lang('ประกาศซื้อ','Buy announcement','购买公告')}}
            </v-btn>
            <v-btn @click="$router.push('/ProductSell')" small text dark>
                <v-icon>mdi-shopping-outline</v-icon>{{_lang('ประกาศขาย','Sell announcement','出售公告')}}
            </v-btn>
