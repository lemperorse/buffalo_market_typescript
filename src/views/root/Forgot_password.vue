...
<div class="h-full w-full flex flex-col justify-center items-center  pt-10 pb-10 md:pb-0 md:pt-0" >
    <v-card class="m-6 md:w-96 rounded-lg">
        <v-card-title class="bgbtn">
            <h2 class="text-white">{{_lang('ลืมรหัสผ่าน','Forgot password','忘記密碼')}}</h2>
        </v-card-title>
        <v-divider></v-divider>
        <v-card-text>
            <form v-if="step==1" @submit.prevent="foundUser()">
                <div > 
                    <v-text-field v-model="form.personal_id" required type="text" :label="_lang('กรอกเลขบัตรประจำตัวประชาชน','ID card number','身份證號碼')" filled rounded prepend-inner-icon="mdi-card-account-details-outline"></v-text-field>
                    <v-text-field v-model="form.tel" required type="text" :label="_lang('กรอกเบอร์โทร','Phone number','電話號碼')" filled rounded prepend-inner-icon="mdi-cellphone"></v-text-field>
                </div> 
                <v-btn rounded large class="w-full bgbtn" type="submit"  dark>{{_lang('ยืนยัน','Confirm','確認')}}</v-btn>
            </form>
